...
{
  Add_Attr (Win, Line_Num, 2, Prompt, SMG$M_BOLD);
  Refresh (Win);
}
...
int
ListRead (ListLength, includefirst, reverse)
      ...
      if (ShowRead || !rflg)
        {
          ...
          rms_status = sys$asctim (0, &atime, &fileio.record.posted, 0);
          if ((rms_status & 1) != 1)
          ...
          if (reverse)
            {
              ...
              if (strncmp (PreviousReplyTop, fileio.record.replyto, SIZE_UID)
                  != 0)
                strcpy (indent, "");
              else
                strcpy (indent, "   ");
              ...
              if (Headr[0] != 0)
                {
                  PutOnTop (&tempptr);
                  sprintf (tempptr->Prompt, "%s%s%s", Headr, indent, Tailr);
          ...
          else
            {
              ...
              if (strncmp (PreviousReplyBot, fileio.record.replyto, SIZE_UID)
                  == 0)
                strcpy (indent, "   ");
              else
                strcpy (indent, "");
              ...
              sprintf (tempptr->Prompt, "%c%.*s %.*s%s%.*s",
                       ...
                       SIZE_TIMESTR, timestr,
                       SIZE_UNAME, fileio.record.user_name,
                       indent, SIZE_SUBJECT, fileio.record.subject);
...
/*Mid Pointer*/ CurrentPtr = LastPtr;
      ...
      if (!rmstest
        ...
        if (rms_status == RMS$_RLK)
          ...
          Status ("Test Point RMSEXP - specified record does not exist.");
      else
        {
          ...
          ListRead (BottomLine -
                    (CurrentLine + TopLine) /* RegionLength/2 */ , FALSE,
                    FALSE);
...
int
ReadNextBlock (Count, Rec, reverse)
     int Count;
     struct rfabuff *Rec;
     int reverse;
{
  ...
  if (reverse && AllReadBot)
    AllReadBot = FALSE;
  if ((!reverse) && AllReadTop)
    AllReadTop = FALSE;
...
*/
  ...
  if (!rmstest (rms_status, 4, RMS$_NORMAL, RMS$_RNF, RMS$_RLK, RMS$_OK_RRL))
    ...
    if (rms_status == RMS$_RLK)
      ...
      Status ("Test Point 1 RMSEXP - specified record does not exist.");
  else
    {
      ...
      if (!rmstest
        ...
        if (rms_status == RMS$_RLK)
          ...
          Status ("Test Point 2 RMSEXP - specified record does not exist.");
      else
        {
          ...
          for (number_elements = 0; number_elements < Count;
               /* number_elements++ */ )
            {
              ...
              if (!rmstest
                  (rms_status, 3, RMS$_NORMAL, RMS$_EOF, RMS$_OK_RRL))
                error_exit ("$GET");
              else if (rms_status == RMS$_EOF)
                {
                  if (reverse)
                  ...
                  if (reverse)
                    {
                      ...
                      if (strncmp
                          (PreviousReplyTop, fileio.record.replyto,
                           SIZE_UID) != 0)
                        strcpy (indent, "");
                      else
                        strcpy (indent, "   ");
                      strncpy (PreviousReplyTop, fileio.record.replyto,
                               SIZE_UID);
                      ...
                      sprintf (Tailr, "%.*s", SIZE_SUBJECT,
                               fileio.record.subject);
                      rfa_copy (&rfar, &rab.rab$w_rfa);
                  ...
                  else
                    {
                      ...
                      if (strncmp
                          (PreviousReplyBot, fileio.record.replyto,
                           SIZE_UID) == 0)
                        strcpy (indent, "   ");
                      else
                        strcpy (indent, "");
                      strncpy (PreviousReplyBot, fileio.record.replyto,
                               SIZE_UID);
...
get_username (void)
{
  ...
  rms_status = lib$getjpi (&item_code, 0, 0, 0, &user_name_d, &lnth);   /*user_name_d.dsc$w_length); */

/*      user_name[user_name_d.dsc$w_length] = 0; */

  for (i = lnth - 1; user_name[i] == ' '; i--)
    user_name[i] = 0;
          ...
          if (quote)
            {
              if (first)
                {
                  sprintf ((*ptr)->storage, "> %.*s wrote..", SIZE_UNAME,
                           fileio.record.user_name);
                  first = FALSE;
                  allocate_line (ptr, &first_line);
                }
                ...
                case 'y':
                  ...
                  if (FirstPtr == NULL)
                  ...
                  else
                    {
                      ...
                      if (CurrentLine < SnapTo)
                        CurrentLine =
                          (SnapTo < ListCount) ? SnapTo : ListCount;
     ...
     int update;
{
  ...
  if (update)
    {
      if (strncmp (fileio.record.user_name, user_name, strlen (user_name)) !=
          0)
        {
  ...
  if (!update)
    {
      ...
      rms_status = lib$getjpi (&item_code,
                               0,
                               0, 0, &user_name_d, &user_name_d.dsc$w_length);
  ...
  rms_status = sys$asctim (0, &timestrdsc, &fileio.record.posted, 0);
  if ((rms_status & 1) != 1)
    lib$signal (rms_status);
     ...
     struct rfabuff *Rec;
{
  ...
  if (!rmstest
      (rms_status, 5, RMS$_NORMAL, RMS$_RNF, RMS$_RLK, RMS$_OK_RRL, RMS$_EOF))
    error_exit ("$GET");
  else if (rmstest (rms_status, 2, RMS$_RNF, RMS$_RLK))
    if (rms_status == RMS$_RLK)
      Status ("Record Locked");
  ...
  do
    {
      ...
      sprintf (outbuffer, "Sender  : %.*s", SIZE_UNAME,
               fileio.record.user_name);
      put_line (Standard_Window, outbuffer);
...
void
active_align_list (CPtr, FindRec)
     struct List_Element **CPtr;
     struct rfabuff *FindRec;
{
  int i;
...
PF4key (void)
{
  ...
  if (!ShowRead)
    return (FALSE);             /* Do Not Allow delete in Unread mode */
  ...
  if (strcmp (user_name, PRIV_USER) != 0)
    {
      ...
      if (!rmstest (rms_status, 2, RMS$_NORMAL, RMS$_OK_RRL))
        {
          warn_user ("RMS error %u  %s %u", rms_status, filterfn (__FILE__),
                     __LINE__);
          return (FALSE);
        }
      ...
      if (strncmp (fileio.record.user_name, user_name, strlen (user_name)) !=
          0)
        {
          warn_user ("Not Authorised to Delete this Post %s %u",
...
Read_Messages (void)
{
  ...
  while (!exit)
    {
      Help
        ("|^P Post   |^R Reply  |^E Edit   |^D Delete |^W Reindex|^U Un/Read| PgUp/PgDn  ^Z to exit");
          ...
          switch (c)
            ...
            case SMG$K_TRM_DOWN:
              ...
              if (CurrentPtr->Next == NULL)     /* ZEN 23/10/2023  */
                if (!ReadNextBlock (5, &CurrentPtr->rfa, FALSE))
                  break;
              LoLite (Standard_Window, CurrentPtr->Prompt,
                      CurrentLine + TopLine);
            ...
            case SMG$K_TRM_NEXT_SCREEN:
              ...
              for (i = 0; i <= BottomLine - TopLine; i++)
                {
                  if (CurrentPtr->Next == NULL)
                    if (!ReadNextBlock (RegionLength, &LastPtr->rfa, FALSE))
                      break;
                  CurrentPtr = CurrentPtr->Next;
                }
              Redraw_Screen ();
            ...
            case SMG$K_TRM_PREV_SCREEN:
              ...
              for (i = 0; i <= (BottomLine - TopLine) + CurrentLine; i++)
                {
                  if (CurrentPtr->Prev == NULL)
                    if (!ReadNextBlock (RegionLength, &FirstPtr->rfa, TRUE))
                      break;
                  CurrentPtr = CurrentPtr->Prev;
                }
              for (i = 0; i < CurrentLine; i++)
